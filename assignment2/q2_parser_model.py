...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embeddings = tf.Variable(self.pretrained_embeddings) # create an embedding tensor and intialize it
        embeddings_looked_up = tf.nn.embedding_lookup(embeddings, self.input_placeholder) 
        embeddings_looked_up = tf.reshape(embeddings_looked_up, shape=[-1, self.config.n_features*self.config.embed_size])
        
        ### END YOUR CODE
        return embeddings_looked_up
