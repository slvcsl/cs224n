...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) > max_length:
            ...
            # pad if shorter
            M = len(sentence)
            sentence = sentence + [zero_vector]*(max_length-M)
            labels = labels + [zero_label]*(max_length-M)
            mask = [True]*M + [False]*(max_length-M)
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables() #reuse Wh, Wx
                o_t, state = cell(x[:, time_step, :], state) # x = (None, max_length, n_features)
    ...
    def add_loss_op(self, preds):
        ...
        masked_logit = tf.boolean_mask(preds, self.mask_placeholder)
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=masked_labels, logits=masked_logit))
        ### END YOUR CODE
        return loss
